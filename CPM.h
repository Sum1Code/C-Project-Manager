...
typedef enum directory_operation { DIR_CREATE, DIR_DELETE, DIR_CHECK } dirOps_e;
...
/*gets the inner string from string struct, note return value is supposed to be
 * immutable*/
const char *string_get(String_t *string) { return string->m_inner_ptr; }
...
void cpm_log(logLevel_e loglvl, const char *fmt, ...) {
  ...
  char timestr[128];
  snprintf(timestr, 128, "(%02d:%02d:%02d)", time_info->tm_hour,
...
void cpm_configure_compiler(BuildProperties_t *bp, const char *compiler,
                            const char *sources, const char *Include_path,
                            const char *build_path, const char *name,
  ...
  if (NULL != compiler || NULL != sources || NULL != name) {
    ...
    cpm_log(CPM_LOG_ERROR,
            "compiler, sourcesPath, and object_name should not be NULL\n");
    exit(1);
  }
...
void cpm_configure_linker(BuildProperties_t *bp, const char *linker,
                          const char *library_path,
                          const char *additional_library,
                          const char *linker_flags) {
  if (!bp->compiler_configured) {
    cpm_log(CPM_LOG_ERROR,
            "please run cpm_configure_compiler before cpm_configure_linker\n");
    exit(1);
  }
...
void cpm_build(BuildProperties_t *bp) {
  if (1 != bp->compiler_configured) {
    cpm_log(CPM_LOG_ERROR,
            "buildProperties hasn't been configured properly!\n");
...
void cpm_link(BuildProperties_t *bp) {
  int compiler_is_linker =
      strcmp(bp->compiler.m_inner_ptr, bp->linker.m_inner_ptr);
#ifndef CPM_SUPPRESS_LINKER_WARNING
  if (0 != compiler_is_linker) {
    cpm_log(CPM_LOG_WARNING, "using custom linker, you will need to provide "
                             "all extra arguments from the linker_flag\n");
    cpm_log(CPM_LOG_WARNING,
            "supress this warning by defining CPM_SUPPRESS_LINKER_WARNING "
            "before including CPM.h\n\n");
  }
#endif
...
void cpm_build_async(BuildProperties_t *bp) { NOT_IMPLEMENTED }
void cpm_build_async_poll(BuildProperties_t *bp) { NOT_IMPLEMENTED }
...
bool dir_ops(dirOps_e directory_operations, const char *dir_path) {
  ...
  case DIR_CREATE: {
    ...
    if (-1 == stat(dir_path, &st)) {
      ...
      cpm_log(CPM_LOG_ERROR, "Directory already existed\n");
      return true;
    }
  } break;
  case DIR_DELETE: {
    if (rmdir(dir_path) == 0) {
      cpm_log(CPM_LOG_INFO, "deleted: %s\n", dir_path);
      return 1;
    } else {
      cpm_log(CPM_LOG_ERROR, "failed to delete %s\n", dir_path);
      return 0;
    }
  } break;
  default:
...
void git_clone(const char *repo_url, const char *target_dir) {
  ...
  if (system(string_get(&cmd))) {
    cpm_log(CPM_LOG_WARNING,
            "Git failed to clone %s maybe the repo already existed!", repo_url);
  }
}

void git_clone_folder(const char *repo_url, const char *folder_name,
                      bool clone_root) {
  NOT_IMPLEMENTED;
}
...
#define CPM_REBUILD_SELF(argc, argv)                                           \
  if (cmp_file_modtime(__FILE__, argv[0])) {                                   \
    ...
    cpm_configure_compiler(&_rebuild_prop, "cc", __FILE__, NULL, NULL,         \
                           GET_FILE_NAME(argv[0]), NULL);                      \
